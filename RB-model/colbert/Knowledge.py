...
class Knowledge(object):
    def __init__(self, data_path: str, tables_json_path: str, databases_path: str, save_path: str):
        """
        :param data_path: 需要预测的数据，按照官网约定为dev/test.json
        :param tables_json_path: 关于表/字段的简称/全称/主键/外键信息，根据官网约定通常为dev_tables.json/test_tables.json
        :param databases_path: 数据库文件夹，应包含database_description 和 .sqlite信息
        :param save_path: 知识保存路径
        """
    ...
    def generate_knowledge(self, data_path, tables_json_path, databases_path, save_path):
        ...
        with open(data_path, 'r', encoding='utf8') as pre_file:
            ...
            for line in tqdm(pre_data, desc='知识预生产'):
                db_name = line['db_id']
...
if __name__ == '__main__':
    knowledge = Knowledge('./data/dev.json', './data/dev_tables.json','./data/dev_databases','./data/pre_knowledge/knowledge.json')
